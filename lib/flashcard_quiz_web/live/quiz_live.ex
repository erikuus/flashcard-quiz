defmodule FlashcardQuizWeb.QuizLive do
  ...
  defp markdown_to_html(text) do
    ...
    |> String.replace(
      ~r/```elixir\n(.*?)\n```/s,
      "<pre class=\"bg-slate-100 p-4 rounded-lg overflow-x-auto\"><code class=\"language-elixir text-sm\">\\1</code></pre>"
    )
    |> String.replace(
      ~r/```(\w+)?\n(.*?)\n```/s,
      "<pre class=\"bg-slate-100 p-4 rounded-lg overflow-x-auto\"><code class=\"text-sm\">\\2</code></pre>"
    )
    |> String.replace(
      ~r/`([^`]+)`/,
